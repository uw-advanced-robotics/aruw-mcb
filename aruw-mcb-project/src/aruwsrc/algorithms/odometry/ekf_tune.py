...
PRIMARY_SAMPLE_PATH = "tools/Forwad 400 CM.csv"
SECONDARY_SAMPLE_PATH = "tools/Forwad 413 beyblading CM.csv"
COLUMN_MAP = {
    "time": "Time",
    "wheel0": "wheel0",
    "wheel1": "wheel1",
...
def wheel_speed_model(
    ...
    ex = math.cos(wheel.angle_rad)
    ey = math.sin(wheel.angle_rad)
    omega_term = yaw_rate * rotation_radius
    return ex * vx_c + ey * vy_c + omega_term
...
def measurement_model_and_jacobian(
    ...
    for i, wheel in enumerate(wheels):
        ...
        vy_c = -s * vx + c * vy
        omega_term = yaw_rate * rotation_radius
        w_hat = ex * vx_c + ey * vy_c + omega_term

        h[i] = w_hat
        H[i, 2] = ex * d_vx_c_d_vx + ey * d_vy_c_d_vx
        H[i, 3] = ex * d_vx_c_d_vy + ey * d_vy_c_d_vy
        H[i, 4] = ex * d_vx_c_d_yaw + ey * d_vy_c_d_yaw
        H[i, 5] = rotation_radius
...
def make_wheels() -> Tuple[List[WheelConfig], float]:
    ...
    width_y = 0.33
    wheels = [
        WheelConfig(wheel_radius,  width_x * 0.5,  width_y * 0.5,  math.pi / 4.0),
        WheelConfig(wheel_radius, -width_x * 0.5,  width_y * 0.5, -math.pi / 4.0),
        WheelConfig(wheel_radius,  width_x * 0.5, -width_y * 0.5,  3.0 * math.pi / 4.0),
        WheelConfig(wheel_radius, -width_x * 0.5, -width_y * 0.5, -3.0 * math.pi / 4.0),
    ]
    rotation_radius = max(math.hypot(w.pos_x, w.pos_y) for w in wheels)
