    ...
    if (capBank != nullptr)
    {
        if (capBank->isOnline())
        {
            capBankBackgroundLine.graphicData.operation =
                capBankBackgroundLine.graphicData.operation == Tx::GRAPHIC_DELETE ? Tx::GRAPHIC_ADD
                                                                               : Tx::GRAPHIC_MODIFY;
            ...
            if (capBankBackgroundLine.graphicData.color != static_cast<uint8_t>(this->previousColor))
            {
                this->previousColor = static_cast<Tx::GraphicColor>(capBankBackgroundLine.graphicData.color);
                RF_CALL(refSerialTransmitter.sendGraphic(&capBankBackgroundLine));
            }
            RF_CALL(refSerialTransmitter.sendGraphic(&capBankVoltageLevel));
        }
