...
namespace aruwsrc::control::client_display 
{
    ...
    Timer::Timer(RefSerialTransmitter& refSerialTransmitter, const RefSerial& refSerial)
        : HudIndicator(refSerialTransmitter), numberIndicator(refSerialTransmitter, &numberGraphic, updateTimer, 0), refSerial(refSerial)
    {
    ...
    {
        timer = tap::arch::clock::getTimerMilliseconds() / 1000 - startTime;
        RF_BEGIN(1);
        numberIndicator.setIndicatorState(timer);
        RF_CALL(numcerIndicator.draw());
        RF_END();
    }
    ...
    void Timer::initialize()
    {
        ...
        RefSerialTransmitter::configGraphicGenerics(&textGraphic.graphicData, graphicName, Tx::GRAPHIC_ADD, DEFAULT_GRAPHIC_LAYER,
...
} // namespace aruw::control::client_display