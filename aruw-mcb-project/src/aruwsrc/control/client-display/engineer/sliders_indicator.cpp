...
{
    // Update lift
    float liftPosition = gantryLift.getPosition();
    float liftPositionPercent =
        getPercentage(liftPosition, gantryLift.getLowerBound(), gantryLift.getUpperBound());
    uint16_t liftCircleX = START_X + BOUNDING_BOX_WIDTH * liftPositionPercent;
