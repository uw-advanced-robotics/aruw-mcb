...
public:
    ...
    */
    ...
    {
        return (mps / (config.diameter * static_cast<float>(M_PI))) / config.motorGearRatio * 60.0f / config.gearRatio;
    }

    inline float rpmToMps(float rpm) const
    {
        return rpm * config.motorGearRatio / 60.0f * config.gearRatio * (config.diameter * static_cast<float>(M_PI));
    }
