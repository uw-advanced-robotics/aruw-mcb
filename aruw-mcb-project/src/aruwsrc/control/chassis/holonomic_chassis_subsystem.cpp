...
{
    if (translationController && translationController->isFinished())
        translationController = nullptr;
    if (yawController && yawController->isFinished()) yawController = nullptr;
    ...
    yawVelRamp.update(FLT_MAX);  // TODO: ACCEPT CONFIG
