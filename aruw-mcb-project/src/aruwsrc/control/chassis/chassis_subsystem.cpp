...
{
    bool refereeSystemConnected = drivers->refSerial.isRefSerialOnline();

    float allMotorCurrentLimit;
    float chassisPowerBuffer = drivers->refSerial.getRobotData().chassis.powerBuffer;
    ...
    if (refereeSystemConnected && chassisPowerBuffer < MIN_POWER_BUFFER_BEFORE_LIMITING)
    {
        ...
        float chassisPowerFractionLimit = aruwlib::algorithms::limitVal<float>(
            (chassisPowerBuffer * chassisPowerBuffer) /
                (static_cast<float>(
                    MIN_POWER_BUFFER_BEFORE_LIMITING * MIN_POWER_BUFFER_BEFORE_LIMITING)),
            0.0f,
            1.0f);
    ...
    float allMotorCurrentOutput =
        abs(leftFrontMotor.getOutputDesired()) + abs(leftBackMotor.getOutputDesired()) +
        abs(rightFrontMotor.getOutputDesired()) + abs(rightBackMotor.getOutputDesired());
    ...
    if (allMotorCurrentOutput > allMotorCurrentLimit && allMotorCurrentOutput != 0)
    {
        ...
        rightFrontMotor.setDesiredOutput(
            rightFrontMotor.getOutputDesired() * chassisOutputFraction);
