...
bool TurretUserControlCommand::isReady()
{
    ...
    return !this->isFinished() && yawController->isOnline() && pitchController->isOnline();
...
void TurretUserControlCommand::execute()
{
    ...
    const float yawInput = controlOperatorInterface.getTurretYawInput(turretID);
    const float pitchInput = controlOperatorInterface.getTurretPitchInput(turretID);
    ...
    const WrappedFloat pitchSetpoint = pitchController->getSetpoint() + (userPitchInputScalar * pitchInput);
