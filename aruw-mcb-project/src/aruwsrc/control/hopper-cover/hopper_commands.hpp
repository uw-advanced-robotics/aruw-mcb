...
public:
    ...
    static constexpr uint32_t STANDARD_OPEN_HOPPER_ANGULAR_SPEED = 5.0f * static_cast<float>(M_PI) * 1000.0f;
...
public:
    ...
    // 5000 milliradians/second
    static constexpr uint32_t STANDARD_CLOSE_HOPPER_ANGULAR_SPEED = 5.0f * static_cast<float>(M_PI) * 1000.0f;
    // Allowable error in radians within which motor will consider target angle reached.
