...
extern "C" {
    ...
    struct SEGGER_RTT_CB {
        ...
        struct {
            const char* sName;              // Optional name. Standard names so far are: "Terminal", "SysView", "J-Scope_i4i4i4"
            char* pBuffer;                  // Pointer to start of buffer
            unsigned int SizeOfBuffer;      // Buffer size in bytes. Note that one byte is lost, as only (SizeOfBuffer-1) bytes can be stored in the buffer.
            unsigned int WrOff;             // Position of next item to be written by either target.
            unsigned int RdOff;             // Position of next item to be read by host. Must be volatile since it may be modified by host.
        ...
        struct {
            ...
            unsigned int RdOff;             // Position of next item to be read by host. Must be volatile since it may be modified by host.
            unsigned int Flags;             // Contains configuration flags
        } aDown[2];                         // Down buffers, transferring information down from host via debug probe to target
    };
