...
public:
    inline InterRobotSignalMessageTransmitter(tap::Drivers &drivers, std::vector<tap::communication::serial::RefSerialData::RobotId> targetIds, uint16_t messageId)
...
private:
    ...
    */
    inline bool getNextMessageToSend()
    {
        ...
        auto nextMessageType = [](MSG_TYPE_ENUM type) {
            return static_cast<MSG_TYPE_ENUM>((static_cast<uint8_t>(type) + 1) % NUM_MSG_TYPES);
        };

        while ((queuedMessageTypeBitmap & (1 << nextMessageType)) == 0)
        {
            nextMessageType = nextMessageType(nextMessageType);
        }
