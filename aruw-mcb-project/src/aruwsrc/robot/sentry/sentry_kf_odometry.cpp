...
SentryKFOdometry::SentryKFOdometry(
    ...
    : drivers(drivers), chassis(chassis), turretMajor(turretMajor), turretMinorLeft(turretMinorLeft), turretMinorRight(turretMinorRight),
    kf(KF_A, KF_C, KF_Q, KF_R, KF_P0)
    { }


float SentryKFOdometry::getChassisYaw() {
    return drivers.mpu6500.getYaw();
}

float SentryKFOdometry::getMajorYaw() {
    return turretMajor.getWorldYaw();
}

float SentryKFOdometry::getLeftMinorYaw() {
    return turretMinorLeft.getWorldYaw();
}

float SentryKFOdometry::getLeftMinorPitch() {
    return turretMinorLeft.getWorldPitch();
}

float SentryKFOdometry::getRightMinorYaw() {
    return turretMinorRight.getWorldYaw();
}

float SentryKFOdometry::getRightMinorPitch() {
    return turretMinorRight.getWorldPitch();
}
