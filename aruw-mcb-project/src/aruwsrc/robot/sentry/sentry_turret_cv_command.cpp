...
void SentryTurretCVCommand::execute()
{
    ...
    targetFound =
        visionCoprocessor.isCvOnline() && !(TurretLeftBallisticsSolution == std::nullopt &&
                                            TurretRightBallisticsSolution == std::nullopt);
    ...
    if (targetFound)
    {
        ...
        if (TurretLeftBallisticsSolution != std::nullopt)
        {
            ...
            TurretLeftYawSetpoint =
                TurretLeftYawSetpoint - sentryTransforms.getWorldToTurretMajor().getYaw();
            TurretLeftPitchSetpoint =
                TurretLeftPitchSetpoint - sentryTransforms.getWorldToTurretMajor().getPitch();
             ...
             */
            TurretLeftYawSetpoint =
                turretLeftSubsystem.yawMotor.unwrapTargetAngle(TurretLeftYawSetpoint);
            TurretLeftPitchSetpoint =
                turretLeftSubsystem.pitchMotor.unwrapTargetAngle(TurretLeftPitchSetpoint);
            ...
            withinAimingToleranceLeft = leftBallisticsSolver.withinAimingTolerance(
                differenceWrappedTurretLeft(
                    yawControllerLeft.getMeasurement(),
                    TurretLeftYawSetpoint),
                differenceWrappedTurretLeft(
                    pitchControllerLeft.getMeasurement(),
                    TurretLeftPitchSetpoint),
        ...
        if (TurretRightBallisticsSolution != std::nullopt)
        {
            ...
            // convert world-relative setpoints to turret major frame setpoint
            TurretRightYawSetpoint =
                TurretRightYawSetpoint - sentryTransforms.getWorldToTurretMajor().getYaw();
            TurretRightPitchSetpoint =
                TurretRightPitchSetpoint - sentryTransforms.getWorldToTurretMajor().getPitch();
             ...
             */
            TurretRightYawSetpoint =
                turretRightSubsystem.yawMotor.unwrapTargetAngle(TurretRightYawSetpoint);
            TurretRightPitchSetpoint =
                turretRightSubsystem.pitchMotor.unwrapTargetAngle(TurretRightPitchSetpoint);
            ...
            withinAimingToleranceRight = rightBallisticsSolver.withinAimingTolerance(
                differenceWrappedTurretRight(
                    yawControllerRight.getMeasurement(),
                    TurretRightYawSetpoint),
                differenceWrappedTurretRight(
                    pitchControllerRight.getMeasurement(),
                    TurretRightPitchSetpoint),
        ...
        WrappedFloat majorYawWrapped =
            WrappedFloat(TurretLeftYawWrapped.minDifference(TurretRightYawWrapped), -M_PI, M_PI);
        majorYawWrapped = WrappedFloat(majorYawWrapped.getWrappedValue() / -2.0f, -M_PI, M_PI);
        majorYawWrapped = WrappedFloat(
            majorYawWrapped.getWrappedValue(),
            0,
            M_TWOPI);  // used wrapped values for majorYawWrapped
    ...
    else
    {
        ...
        else
        {
            ...
            float v = majorScanValue.getWrappedValue();  // guessed wrapped
            ...
            majorSetpoint = lowPassFilter(
                majorSetpoint,
                majorScanValue.getWrappedValue(),
                SCAN_LOW_PASS_ALPHA);  // guessed wrapped
...
{
    return !yawControllerLeft.isOnline() || !pitchControllerLeft.isOnline() ||
           !yawControllerRight.isOnline() || !pitchControllerRight.isOnline();
}
