...
void SentryTurretCVCommand::execute()
{
    ...
    auto malewifeBallisticsSolution = malewifeBallisticsSolver.computeTurretAimAngles();

    if (ignoreTargetTimeout.isExpired() && malewifeBallisticsSolution != std::nullopt)
    {
        exitScanMode();
