...
public:
     ...
     * @param[in] energy_limit The limit in energy allowed to hit the hardstop, set to 0 to not allow any.
     * @param[in] maxTorque The maximum torque allowed to be applied to the motors.
     */
    LegHardstopCBF(tap::algorithms::CMSISMat<2,2>& jacobianTranspose, BalstdLegState& LegState, const BalstdLegConfig config, float energy_limit, float maxTorque);
