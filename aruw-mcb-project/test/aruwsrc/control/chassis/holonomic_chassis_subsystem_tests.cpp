...
TEST_F(HolonomicChassisSubsystemTest, getDesiredRotation_returns_desired_rotation)//4 motor, swerve
{
    chassis.setDesiredOutput(0, 0, CHASSIS_VEL);
    EXPECT_NEAR(CHASSIS_VEL, chassis.getDesiredRotation(), 1E-3);
}

TEST_F(HolonomicChassisSubsystemTest, setZeroRPM_doesnt_reset_desired_velocity)//4 motor
{
    chassis.setDesiredOutput(0, 0, CHASSIS_VEL);
    chassis.setZeroRPM();
    EXPECT_NEAR(CHASSIS_VEL, chassis.getDesiredRotation(), 1E-3);
...
TEST_F(HolonomicChassisSubsystemTest, allMotorsOnline)//4 motor, swerve
{
...
TEST_F(HolonomicChassisSubsystemTest, onHardwareTestStart_sets_desired_out_0)//swerve, 4 motor
{
...
TEST_F(HolonomicChassisSubsystemTest, getRightBackRpmActual)//4 motor
{
    ON_CALL(chassis.rightBackMotor, getShaftRPM).WillByDefault(Return(1000));
    EXPECT_NEAR(1000, chassis.getRightBackRpmActual(), 1E-3);
}

TEST_F(HolonomicChassisSubsystemTest, initialize)//4 motor, swerve(done)
{
