...
{
public:
    CapBankTests() : drivers(), capBank(&drivers, tap::can::CanBus::CAN_BUS1, 1.0, 1000), clock() {}
...
TEST_F(CapBankTests, available_supply_power_is_capped_at_max_available_power)
{
    // maxAvailablePower = 20 W; the same STATUS frame reports raw 10 -> 40 W, so the
    // getter (and getMaximumOutputCurrent, which reads through it) must clamp to 20 W.
    CapacitorBank cappedBank(&drivers, tap::can::CanBus::CAN_BUS1, 1.0, 20);
    ...
    message.data[5] = 0x2E;
    message.data[6] = 42;
    message.data[7] = 10;  // raw 10 -> 40 W reported, above the 20 W cap

    cappedBank.processMessage(message);

    EXPECT_EQ(20, cappedBank.getAvailableSupplyPower());
    EXPECT_NEAR(20.0f / 24.0f, cappedBank.getMaximumOutputCurrent(), 1e-3);
}
