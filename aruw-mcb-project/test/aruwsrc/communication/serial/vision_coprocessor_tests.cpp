...
TEST(VisionCoprocessor, send_strategy_message_does_a_thing)
{
    ClockStub clock;
    ...
    VisionCoprocessor::Strategy hideLikeABitch = VisionCoprocessor::Strategy::GoToFriendlyBase;
    ...
    EXPECT_CALL(drivers.uart, write(_, _, MSG_LEN))
        ...
        .WillOnce(
            [&](tap::communication::serial::Uart::UartPort, const uint8_t *data, std::size_t length)
            {
                ...
                checkHeaderAndTail<1>(msg);
                EXPECT_EQ(13, msg.messageType);
                EXPECT_EQ(1, *reinterpret_cast<uint8_t *>(msg.data));

                return length;
            });

    serial.sendMotionStrategyMessage(hideLikeABitch);
}
