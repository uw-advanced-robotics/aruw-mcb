...
namespace control
{
void ToggleCommandMapping::executeCommandMappingRemove(const RemoteMapState &currState)
{
...
void ToggleCommandMapping::executeCommandMappingAdd(const RemoteMapState &currState)
{
    if (!(mapState.getNegKeysUsed() && negKeysSubset(mapState, currState)) && mappingSubset(currState))
    {
        if (!pressed)
        {
            if (!toggled)
