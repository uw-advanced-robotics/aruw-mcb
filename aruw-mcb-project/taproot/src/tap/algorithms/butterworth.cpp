 ...
 */
 ...
 #include <iostream>
 #include <iomanip>
 #include "modm/math/geometry/angle.hpp" // for M_PI 
 ...
 constexpr std::array<double, ORDER + 1>
 expandPolynomial(std::array<std::complex<double>, ORDER> zeros) {
   ...
   for (size_t i = 0; i < ORDER; i++) {
     ...
     std::array<std::complex<double>, ORDER + 1> newCoefficients{
 ...
 template <uint8_t ORDER>
 constexpr double calculateScalar(std::array<double, ORDER + 1> numerator,
 ...
 public:
   ...
   constexpr Butterworth(double wc, double Ts)
       : naturalResponseCoefficients(), forcedResponseCoefficients() {
     int n = ORDER;
     ...
     for (int k = 0; k < static_cast<int>(n); ++k) {
       ...
       std::complex<double> pole =
           std::complex<double>(wc * cos(theta), wc * sin(theta));
       poles[k] = pole;
     }
     ...
     double scale_factor = calculateScalar<ORDER>(forcedResponseCoefficients,
