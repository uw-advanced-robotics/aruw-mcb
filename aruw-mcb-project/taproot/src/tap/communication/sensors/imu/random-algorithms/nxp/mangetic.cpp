    ...
    int32_t loopcounter)
{
    ...
    if ((pthisMagBuffer->iMagBufferCount < MAXMEASUREMENTS) && (pthisMagBuffer->index[j][k] != -1))
    {
        ...
        for (i = CHX; i <= CHZ; i++)
        {
            idelta += abs((int32_t)pthisMag->iBs[i] - (int32_t)pthisMagBuffer->iBs[i][j][k]);
        }
            ...
            while (!itooclose && (j < MAGBUFFSIZEX))
            {
                ...
                while (!itooclose && (k < MAGBUFFSIZEY))
                {
                    ...
                    if (pthisMagBuffer->index[j][k] != -1)
                    {
                        ...
                        {
                            idelta +=
                                abs((int32_t)pthisMag->iBs[i] - (int32_t)pthisMagBuffer->iBs[i][j][k]);
                        }
    ...
    if (pthisMagCal->itimeslice == 0)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            if (pthisMagCal->iSumBs[i] >= 0)
                pthisMagCal->iMeanBs[i] = (pthisMagCal->iSumBs[i] + ((int32_t)iM >> 1)) / (int32_t)iM;
            else
                pthisMagCal->iMeanBs[i] = (pthisMagCal->iSumBs[i] - ((int32_t)iM >> 1)) / (int32_t)iM;
        }
    ...
    else if ((pthisMagCal->itimeslice >= 1) && (pthisMagCal->itimeslice <= MAGBUFFSIZEX))
    {
        ...
        for (j = 0; j < MAGBUFFSIZEY; j++)
        {
            if (pthisMagBuffer->index[i][j] != -1)
            {
                ...
                for (k = 0; k < 3; k++)
                    iBsZeroMean[k] =
                        (int32_t)pthisMagBuffer->iBs[k][i][j] - (int32_t)pthisMagCal->iMeanBs[k];
        ...
        if (pthisMagBuffer->index[i][j] != -1)
        {
            ...
            for (k = 0; k < 3; k++)
            {
                pthisMagCal->fvecA[k + 3] =
                    (float)((int32_t)pthisMagBuffer->iBs[k][i][j] - (int32_t)pthisMagCal->iMeanBs[k]);
    ...
    for (j = 0; j < MAGBUFFSIZEX; j++)
    {
        for (k = 0; k < MAGBUFFSIZEY; k++)
        {
            if (pthisMagBuffer->index[j][k] != -1)
            {
                ...
                {
                    pthisMagCal->fvecA[l] =
                        (float)((int32_t)pthisMagBuffer->iBs[l][j][k] - (int32_t)iOffset[l]) * fscaling;
