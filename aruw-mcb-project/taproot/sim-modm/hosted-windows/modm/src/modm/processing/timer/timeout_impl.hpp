...
template <class Clock, class Duration>
void modm::GenericTimeout<Clock, Duration>::restart()
{
    restart(_interval);
}
...
template <class Clock, class Duration>
template <typename Rep, typename Period>
void modm::GenericTimeout<Clock, Duration>::restart(std::chrono::duration<Rep, Period> interval)
{
    if (0 <= interval.count())
    {
        const auto cast_interval = std::chrono::duration_cast<duration>(interval);
        modm_assert_continue_fail_debug(
            interval.count() <= duration::max().count(),
            "tmr.size",
            "Timer interval must be smaller than the maximal duration!",
            interval.count());
...
template <class Clock, class Duration>
void modm::GenericTimeout<Clock, Duration>::stop()
{
    _state = STOPPED;
...
{
    if (_state & (EXECUTED | STOPPED)) return false;
...
template <class Clock, class Duration>
typename modm::GenericTimeout<Clock, Duration>::wide_signed_duration modm::
    GenericTimeout<Clock, Duration>::remaining() const
{
    if (_state == STOPPED) return wide_signed_duration{0};
...
template <class Clock, class Duration>
typename modm::GenericTimeout<Clock, Duration>::duration modm::GenericTimeout<Clock, Duration>::
    interval() const
{
    return _interval;
}
...
// ----------------------------------------------------------------------------
template <class Clock, class Duration>
modm::TimerState modm::GenericTimeout<Clock, Duration>::state() const
{
    if (checkExpiration()) _state = (_state & ~ARMED) | EXPIRED;
...
// ----------------------------------------------------------------------------
template <class Clock, class Duration>
bool modm::GenericTimeout<Clock, Duration>::checkExpiration() const
{
    return (_state & ARMED) and (now() - _start) >= _interval;
}

template <class Clock, class Duration>
typename modm::GenericTimeout<Clock, Duration>::time_point modm::GenericTimeout<Clock, Duration>::
    now() const
{
