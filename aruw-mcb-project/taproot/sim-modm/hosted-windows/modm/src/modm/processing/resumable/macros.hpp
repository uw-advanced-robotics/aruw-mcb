...
#define RF_END_RETURN(...)                           \
    ...
    default:                                         \
        ...
        static_assert(                               \
            uint16_t(__COUNTER__) - rfCounter < 256, \
            "You have too many states in this resumable function!")
...
/// @hideinitializer
#define RF_YIELD() RF_INTERNAL_SET_CASE_YIELD(__COUNTER__)
...
/// @hideinitializer
#define RF_WAIT_UNTIL(...) RF_WAIT_WHILE(!(__VA_ARGS__))
...
#define RF_ISEMPTY(...)                                                                           \
    _RF_ISEMPTY(/* test if there is just one argument, eventually an empty one */                 \
                RF_HAS_COMMA(__VA_ARGS__), /* test if _TRIGGER_PARENTHESIS_ together with the     \
                                              argument adds a comma */                            \
                RF_HAS_COMMA(                                                                     \
                    RF_TRIGGER_PARENTHESIS_ __VA_ARGS__), /* test if the argument together with a \
                                                             parenthesis adds a comma */          \
                RF_HAS_COMMA(                                                                     \
                    __VA_ARGS__(/*empty*/)), /* test if placing it between _TRIGGER_PARENTHESIS_  \
