...
uint8_t modm::GraphicDisplay::getFontHeight(const modm::accessor::Flash<uint8_t>* font)
{
...
uint16_t modm::GraphicDisplay::getStringWidth(
    const char* s,
    const modm::accessor::Flash<uint8_t>* font)
{
    if (!font->isValid()) return 0;
...
void modm::GraphicDisplay::write(char c)
{
    ...
    this->drawImageRaw(cursor, width, height, accessor::asFlash(font.getPointer() + offset));
...
// ----------------------------------------------------------------------------
void modm::GraphicDisplay::Writer::write(char c) { this->parent->write(c); }

void modm::GraphicDisplay::Writer::flush() {}

bool modm::GraphicDisplay::Writer::read(char&) { return false; }
