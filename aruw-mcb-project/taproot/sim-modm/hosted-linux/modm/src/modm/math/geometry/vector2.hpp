...
protected:
    template <typename U>
    friend IOStream& operator<<(IOStream& os, const Vector<U, 2>& c);

    template <typename U>
    friend Vector<U, 2> operator*(float scale, const Vector<U, 2>& vector);
};
...
template <>
template <typename U>
Vector<U, 2> Vector<float, 2>::convert() const
{
...
template <>
template <typename U>
Vector<U, 2> Vector<double, 2>::convert() const
{
    return Vector<U, 2>(round(this->x), round(this->y));
}
}  // namespace modm
