...
// ----------------------------------------------------------------------------
template <typename T, template <typename> class Accessor>
typename modm::interpolation::Linear<T, Accessor>::OutputType modm::interpolation::
    Linear<T, Accessor>::interpolate(const InputType& value) const
{
    ...
    for (i = 1; i < this->numberOfPoints; ++i)
    {
        ...
        if (value <= current.getFirst())
        {
            ...
            WideType b =
                static_cast<OutputSignedType>(x2_out) - static_cast<OutputSignedType>(x1_out);
