 ...
 * \endcode
 */
modm_always_inline uint8_t swap(uint8_t n)
{
#ifdef MODM_CPU_AVR
...
/// Exchange two byte
modm_always_inline void swap(uint8_t& a, uint8_t& b)
{
    uint8_t temp = a;
    a = b;
    b = temp;
 ...
 * \endcode
 */
modm_always_inline uint16_t swap(uint16_t n)
{
#ifdef MODM_CPU_ARM
    asm volatile(
        "rev16 %0,%0"
 ...
 * \endcode
 */
modm_always_inline uint32_t swap(uint32_t n)
{
...
modm_always_inline void swap(int16_t& a, int16_t& b)
{
    int16_t temp = a;
    a = b;
    b = temp;
 ...
 */
inline uint16_t bitReverse(uint16_t n)
{
#if defined(MODM_CPU_ARM) && !defined(MODM_CPU_CORTEX_M0)
    asm volatile(
        "rbit %0,%0"
...
/// @cond
constexpr int8_t leftmostBitHelper(uint32_t value)
{
...
constexpr int8_t leftmostBit(uint32_t value) { return leftmostBitHelper(value) - 1; }
 ...
 */
std::size_t bitCount(uint16_t n);

/// Count the number of bits set
std::size_t bitCount(uint32_t n);
