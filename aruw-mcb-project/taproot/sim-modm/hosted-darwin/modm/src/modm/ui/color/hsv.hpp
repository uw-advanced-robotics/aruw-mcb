...
public:
    ...
    requires std::is_same_v<T, uint16_t>&& std::is_same_v<U, uint8_t> constexpr HsvT(
        const HsvT<U>& hsv_other)
        : hue(hsv_other.hue << 8),
     ...
     */
    template <std::unsigned_integral U>
    requires std::is_same_v<T, uint8_t>&& std::is_same_v<U, uint16_t> constexpr HsvT(
        const HsvT<U>& hsv_other)
        : hue(hsv_other.hue >> 8),
          saturation(hsv_other.saturation >> 8),
          value(hsv_other.value >> 8)
