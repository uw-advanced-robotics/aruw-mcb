...
public:
    ...
    requires std::is_same_v<T, uint16_t>&& std::is_same_v<U, uint8_t> constexpr RgbT(
        const RgbT<U>& rgb_other)
        : red(rgb_other.red << 8),
     ...
     */
    template <std::unsigned_integral U>
    requires std::is_same_v<T, uint8_t>&& std::is_same_v<U, uint16_t> constexpr RgbT(
        const RgbT<U>& rgb_other)
        : red(rgb_other.red >> 8),
          green(rgb_other.green >> 8),
          blue(rgb_other.blue >> 8)
...
requires std::is_fundamental_v<IntermediateType> constexpr RgbT<ReturnType> normalizeColor(
    RgbT<T> rgb,
