...
public:
    ...
    requires std::is_same_v<T, uint16_t> &&std::is_same_v<U, uint8_t> constexpr BrightnessT(
        const BrightnessT<U> &brightness_other)
     ...
     */
    template <std::unsigned_integral U>
    requires std::is_same_v<T, uint8_t> &&std::is_same_v<U, uint16_t> constexpr BrightnessT(
        const BrightnessT<U> &brightness_other)
        : value(brightness_other.value >> 8)
    {
    }
