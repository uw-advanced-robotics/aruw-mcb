...
public:
    ...
    inline xpcc::Communicator* getCommunicator();

protected:
    inline void callAction(uint8_t receiver, uint8_t actionIdentifier);

    inline void callAction(
        uint8_t receiver,
        uint8_t actionIdentifier,
        ResponseCallback& responseCallback);

    template <typename T>
    inline void callAction(uint8_t receiver, uint8_t actionIdentifier, const T& data);
    ...
    inline void publishEvent(uint8_t eventIdentifier);

    template <typename T>
    inline void publishEvent(uint8_t eventIdentifier, const T& data);

    inline void sendResponse(const ResponseHandle& handle);

    template <typename T>
    inline void sendResponse(const ResponseHandle& handle, const T& data);

    template <typename T>
    inline void sendNegativeResponse(const ResponseHandle& handle, const T& data);

    inline void sendNegativeResponse(const ResponseHandle& handle);
