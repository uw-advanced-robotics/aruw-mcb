...
public:
    ...
    static inline bool isFragment(const uint32_t& identifier)
    {
...
protected:
    ...
    {
    public:
        SendListItem(const uint32_t& inIdentifier, const modm::SmartPointer& inPayload)
    ...
    private:
        SendListItem& operator=(const SendListItem& other);
    };

    class ReceiveListItem
    {
    public:
        ReceiveListItem(uint8_t size, const Header& inHeader, uint8_t messageCounter = 0)
            : header(inHeader),
              payload(size),
              receivedFragments(0),
    ...
    private:
        ReceiveListItem& operator=(const ReceiveListItem& other);
    };
