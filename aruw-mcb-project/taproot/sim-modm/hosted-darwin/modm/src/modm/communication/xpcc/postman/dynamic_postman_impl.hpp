...
// ----------------------------------------------------------------------------
template <class C>
bool xpcc::DynamicPostman::registerEventListener(
    ...
    void (C::*memberFunction)(const Header &))
{
    ...
    eventMap.insert(std::pair<uint8_t, EventListener>(
        eventId,
        EventListener(
            static_cast<EventCallbackSimple>(std::bind(memberFunction, componentObject, _1)))));
    ...
    void (C::*memberFunction)(const Header &, const P &))
{
    ...
    eventMap.insert(std::pair<uint8_t, EventListener>(
        eventId,
        EventListener(
            std::bind(reinterpret_cast<Function>(memberFunction), componentObject, _1, _2))));

    return true;
}

template <class C>
bool xpcc::DynamicPostman::registerActionHandler(
    ...
    void (C::*memberFunction)(const ResponseHandle &))
{
    ...
    actionMap[componentId][actionId] = ActionHandler(
        static_cast<ActionCallbackSimple>(std::bind(memberFunction, componentObject, _1)));
