...
void xpcc::Dispatcher::handleActionCall(const Header& header, const modm::SmartPointer& payload)
{
...
void xpcc::Dispatcher::sendAcknowledge(const Header& header)
{
    Header ackHeader(header.type, true, header.source, header.destination, header.packetIdentifier);

    this->backend->sendPacket(ackHeader);
}

bool xpcc::Dispatcher::Entry::headerFits(const Header& inHeader) const
{
    return (
        (inHeader.source == this->header.destination) &&
        (inHeader.destination == this->header.source) &&
        (inHeader.packetIdentifier == this->header.packetIdentifier));
}

bool xpcc::Dispatcher::handlePacket(const Header& header, const modm::SmartPointer& payload)
{
    bool ack = false;
...
xpcc::Dispatcher::EntryIterator xpcc::Dispatcher::sendMessageToInnerComponent(EntryIterator entry)
{
    ...
    else
    {
        ...
        for (++req; req != this->entries.end(); ++req)
        {
            if (req->header.type == Header::Type::REQUEST and
                ...
                req->state != Entry::State::TransmissionPending and req->headerFits(entry->header))
            {
...
void xpcc::Dispatcher::handleWaitingMessages()
{
    auto entry = this->entries.begin();
    while (entry != this->entries.end())
    {
...
// ----------------------------------------------------------------------------
void xpcc::Dispatcher::addMessage(const Header& header, modm::SmartPointer& smartPayload)
{
    this->entries.append(Entry(header, smartPayload));
}

void xpcc::Dispatcher::addMessage(
    const Header& header,
...
void xpcc::Dispatcher::addResponse(const Header& header, modm::SmartPointer& smartPayload)
{
