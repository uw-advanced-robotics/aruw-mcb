...
template <typename T, template <typename> class Accessor>
typename modm::interpolation::Lagrange<T, Accessor>::OutputType modm::interpolation::
    Lagrange<T, Accessor>::interpolate(const InputType& value) const
{
    OutputType ret = 0;
            ...
            if (i != j)
            {
                li *= static_cast<OutputType>(value - this->supportingPoints[j].getFirst()) /
                      static_cast<OutputType>(
                          this->supportingPoints[i].getFirst() -
