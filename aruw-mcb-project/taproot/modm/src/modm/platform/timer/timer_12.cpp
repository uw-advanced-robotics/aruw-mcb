...
// ----------------------------------------------------------------------------
void modm::platform::Timer12::clockEnable() { enable(); }

void modm::platform::Timer12::enable() { Rcc::enable<Peripheral::Tim12>(); }

void modm::platform::Timer12::disable()
{
    TIM12->CR1 = 0;
    TIM12->DIER = 0;
    TIM12->CCER = 0;
...
// ----------------------------------------------------------------------------
void modm::platform::Timer12::setMode(
    Mode mode,
    SlaveMode slaveMode,
    ...
    bool limitUpdateEventRequestSource)
{
    ...
    TIM12->SMCR = static_cast<uint32_t>(slaveMode) | static_cast<uint32_t>(slaveModeTrigger);
}

// ----------------------------------------------------------------------------
void modm::platform::Timer12::configureInputChannel(
    uint32_t channel,
    ...
    bool xor_ch1_3)
{
    ...
    TIM12->CCER |= (TIM_CCER_CC1E | static_cast<uint32_t>(polarity)) << (channel * 4);
}

// ----------------------------------------------------------------------------
void modm::platform::Timer12::configureOutputChannel(
    uint32_t channel,
...
// ----------------------------------------------------------------------------
void modm::platform::Timer12::enableInterruptVector(bool enable, uint32_t priority)
{
    if (enable)
    {
...
// ----------------------------------------------------------------------------
bool modm::platform::Timer12::isChannelConfiguredAsInput(uint32_t channel)
{
    bool isInput = false;
    switch (channel)
    {
