...
public:
    ...
    static inline void pause() { TIM3->CR1 &= ~TIM_CR1_CEN; }

    static inline void start() { TIM3->CR1 |= TIM_CR1_CEN; }
    ...
    static uint16_t getPrescaler() { return (TIM3->PSC + 1); }

    static inline void setOverflow(Value overflow) { TIM3->ARR = overflow; }
    ...
    static inline Value getValue() { return TIM3->CNT; }

    static inline void setValue(Value value) { TIM3->CNT = value; }

    static inline void enableOutput() { TIM3->BDTR |= TIM_BDTR_MOE; }

    static inline void disableOutput() { TIM3->BDTR &= ~(TIM_BDTR_MOE); }
...
public:
    ...
    static inline Value getCompareValue(uint32_t channel) { return *(&TIM3->CCR1 + (channel - 1)); }

public:
    static void enableInterruptVector(bool enable, uint32_t priority);

    static inline void enableInterrupt(Interrupt_t interrupt) { TIM3->DIER |= interrupt.value; }

    static inline void disableInterrupt(Interrupt_t interrupt) { TIM3->DIER &= ~interrupt.value; }

    static inline InterruptFlag_t getEnabledInterrupts() { return InterruptFlag_t(TIM3->DIER); }
    ...
    static inline InterruptFlag_t getInterruptFlags() { return InterruptFlag_t(TIM3->SR); }
