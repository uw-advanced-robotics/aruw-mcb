...
// ----------------------------------------------------------------------------
void modm::platform::Timer3::clockEnable() { enable(); }

void modm::platform::Timer3::enable() { Rcc::enable<Peripheral::Tim3>(); }

void modm::platform::Timer3::disable()
{
    TIM3->CR1 = 0;
    TIM3->DIER = 0;
    TIM3->CCER = 0;
...
// ----------------------------------------------------------------------------
void modm::platform::Timer3::setMode(
    Mode mode,
    SlaveMode slaveMode,
    ...
    bool limitUpdateEventRequestSource)
{
    ...
    TIM3->SMCR = static_cast<uint32_t>(slaveMode) | static_cast<uint32_t>(slaveModeTrigger);
}

// ----------------------------------------------------------------------------
void modm::platform::Timer3::configureInputChannel(
    uint32_t channel,
    ...
    bool xor_ch1_3)
{
    ...
    TIM3->CCER |= (TIM_CCER_CC1E | static_cast<uint32_t>(polarity)) << (channel * 4);
}

// ----------------------------------------------------------------------------
void modm::platform::Timer3::configureOutputChannel(
    uint32_t channel,
...
// ----------------------------------------------------------------------------
void modm::platform::Timer3::enableInterruptVector(bool enable, uint32_t priority)
{
    if (enable)
    {
...
// ----------------------------------------------------------------------------
bool modm::platform::Timer3::isChannelConfiguredAsInput(uint32_t channel)
{
    bool isInput = false;
    switch (channel)
    {
