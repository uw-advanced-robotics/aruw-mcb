...
public:
    ...
    static inline void pause() { TIM12->CR1 &= ~TIM_CR1_CEN; }

    static inline void start() { TIM12->CR1 |= TIM_CR1_CEN; }
    ...
    static uint16_t getPrescaler() { return (TIM12->PSC + 1); }

    static inline void setOverflow(Value overflow) { TIM12->ARR = overflow; }
    ...
    static inline Value getValue() { return TIM12->CNT; }

    static inline void setValue(Value value) { TIM12->CNT = value; }

    static inline void enableOutput() { TIM12->BDTR |= TIM_BDTR_MOE; }

    static inline void disableOutput() { TIM12->BDTR &= ~(TIM_BDTR_MOE); }
...
public:
    ...
    static inline void enableInterrupt(Interrupt_t interrupt) { TIM12->DIER |= interrupt.value; }

    static inline void disableInterrupt(Interrupt_t interrupt) { TIM12->DIER &= ~interrupt.value; }

    static inline InterruptFlag_t getEnabledInterrupts() { return InterruptFlag_t(TIM12->DIER); }
    ...
    static inline InterruptFlag_t getInterruptFlags() { return InterruptFlag_t(TIM12->SR); }
