...
public:
	...
	setPeriod(std::chrono::duration<Rep, Period> duration, bool autoApply = true)
	{
		...
		const uint32_t cycles = duration.count() * SystemClock::Timer12 * Period::num / Period::den;
		uint16_t prescaler;
	...
	{
		TIM12->EGR = static_cast<uint32_t>(ev);
	}
	...
	isCountingUp()
	{
		return (TIM12->CR1 & TIM_CR1_DIR) == 0;
	}
...
public:
	...
	{
		constexpr auto channel = signalToChannel<Peripheral::Tim12, Signal>();
