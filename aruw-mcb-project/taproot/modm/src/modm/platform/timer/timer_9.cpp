...
// ----------------------------------------------------------------------------
void
modm::platform::Timer9::clockEnable()
{
	enable();
}

void
modm::platform::Timer9::enable()
{
	Rcc::enable<Peripheral::Tim9>();
}

void
modm::platform::Timer9::disable()
{
	TIM9->CR1 = 0;
	TIM9->DIER = 0;
	TIM9->CCER = 0;

	Rcc::disable<Peripheral::Tim9>();
}

// ----------------------------------------------------------------------------
void
modm::platform::Timer9::setMode(Mode mode, SlaveMode slaveMode,
...
// ----------------------------------------------------------------------------
void
modm::platform::Timer9::configureInputChannel(uint32_t channel,
...
// ----------------------------------------------------------------------------
void
modm::platform::Timer9::configureOutputChannel(uint32_t channel,
...
// ----------------------------------------------------------------------------
void
modm::platform::Timer9::enableInterruptVector(bool enable, uint32_t priority)
{
	if (enable)
	{
...
// ----------------------------------------------------------------------------
bool
modm::platform::Timer9::isChannelConfiguredAsInput(uint32_t channel)
{
	bool isInput = false;
	switch (channel) {
		case 1:
			isInput = TIM9->CCMR1 & TIM_CCMR1_CC1S;
			break;
		case 2:
			isInput = TIM9->CCMR1 & TIM_CCMR1_CC2S;
			break;
		default:
			break;
	}
