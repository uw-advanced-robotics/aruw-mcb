...
#ifndef MODM_STM32_TIMER_9_HPP
#define MODM_STM32_TIMER_9_HPP
...
public:
	...
	connect()
	{
		using Connector = GpioConnector<Peripheral::Tim9, Signals...>;
		Connector::connect();
	}
	...
	setPeriod(uint32_t microseconds, bool autoApply = true)
	{
		...
		// unequal to zero)
		uint32_t cycles = microseconds * (SystemClock::Timer9 / 1'000'000UL);
	...
	getTickFrequency()
	{
		return SystemClock::Timer9 / (TIM9->PSC + 1);
	}
...
public:
	...
	{
		return InterruptFlag_t(TIM9->DIER);
	}
