...
uint8_t modm::platform::SpiMaster1::state(0);

uint8_t modm::platform::SpiMaster1::count(0);

void *modm::platform::SpiMaster1::context(nullptr);

modm::Spi::ConfigurationHandler modm::platform::SpiMaster1::configuration(nullptr);
// ----------------------------------------------------------------------------

uint8_t modm::platform::SpiMaster1::acquire(void *ctx, ConfigurationHandler handler)
{
    if (context == nullptr)
    {
        context = ctx;
...
uint8_t modm::platform::SpiMaster1::release(void *ctx)
{
    if (ctx == context)
    {
...
modm::ResumableResult<uint8_t> modm::platform::SpiMaster1::transfer(uint8_t data)
{
    ...
    if (!(state & Bit0))
    {
        ...
        if (!SpiHal1::isTransmitRegisterEmpty()) return {modm::rf::Running};
    ...
    if (!SpiHal1::isReceiveRegisterNotEmpty()) return {modm::rf::Running};
...
modm::ResumableResult<void> modm::platform::SpiMaster1::transfer(
    const uint8_t *tx,
    uint8_t *rx,
    std::size_t length)
{
