...
public:
	...
	initialize()
	{
		constexpr auto result = modm::Prescaler::from_power(SystemClock::Spi4, baudrate, 2, 256);
		assertBaudrateInTolerance< result.frequency, baudrate, tolerance >();

		// translate the prescaler into the bitmapping
		constexpr SpiHal4::Prescaler prescaler{result.index << SPI_CR1_BR_Pos};
	...
	{
		SpiHal4::setDataSize(static_cast<SpiHal4::DataSize>(size));
	}
