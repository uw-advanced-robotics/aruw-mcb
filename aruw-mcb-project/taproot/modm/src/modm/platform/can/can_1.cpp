...
// ----------------------------------------------------------------------------
bool modm::platform::Can1::initializeWithPrescaler(
    uint16_t prescaler,
    uint8_t bs1,
    uint8_t bs2,
    ...
    bool overwriteOnOverrun)
{
    ...
    if (deadlockPreventer == 0) return false;
    ...
    CAN1->BTR = ((1 - 1) << CAN_BTR_SJW_POS) |    // SJW (1 to 4 possible)
                ((bs2 - 1) << CAN_BTR_TS2_POS) |  // BS2 Samplepoint
                ((bs1 - 1) << CAN_BTR_TS1_POS) |  // BS1 Samplepoint
                static_cast<uint32_t>(startupMode) | (prescaler - 1);
...
static void sendMailbox(const modm::can::Message& message, uint32_t mailboxId)
{
...
static void readMailbox(modm::can::Message& message, uint32_t mailboxId, uint8_t* filter_id)
{
    ...
    if (filter_id != nullptr) (*filter_id) = (mailbox->RDTR & CAN_RDT1R_FMI) >> CAN_RDT1R_FMI_Pos;
...
MODM_ISR(CAN1_RX0)
{
    if (not modm_assert_continue_ignore(
            not(CAN1->RF0R & CAN_RF0R_FOVR0),
            "can.rx.hw0",
            "CAN receive hardware buffer overflowed!",
            1))
    {
    ...
    modm_assert_continue_ignore(
        rxQueue.push(rxMessage),
        "can.rx.sw0",
        "CAN receive software buffer overflowed!",
        1);
}
...
void modm::platform::Can1::setMode(Mode mode)
{
    ...
    CAN1->BTR = (CAN1->BTR & ~(CAN_BTR_SILM | CAN_BTR_LBKM)) | static_cast<uint32_t>(mode);
...
// ----------------------------------------------------------------------------
void modm::platform::Can1::setAutomaticRetransmission(bool retransmission)
{
    if (retransmission)
    {
...
// ----------------------------------------------------------------------------
bool modm::platform::Can1::isMessageAvailable() { return rxQueue.isNotEmpty(); }

// ----------------------------------------------------------------------------
bool modm::platform::Can1::getMessage(can::Message& message, uint8_t* filter_id)
{
    if (rxQueue.isEmpty())
    {
...
// ----------------------------------------------------------------------------
bool modm::platform::Can1::isReadyToSend() { return txQueue.isNotFull(); }

// ----------------------------------------------------------------------------
bool modm::platform::Can1::sendMessage(const can::Message& message)
{
    ...
    if ((CAN1->TSR & (CAN_TSR_TME0 | CAN_TSR_TME1 | CAN_TSR_TME2)) == 0)
    {
        ...
        if (not modm_assert_continue_ignore(
                ...
                "can.tx",
                "CAN transmit software buffer overflowed!",
                1))
        {
            return false;
        }
...
// ----------------------------------------------------------------------------
modm::platform::Can1::BusState modm::platform::Can1::getBusState()
{
...
// ----------------------------------------------------------------------------
void modm::platform::Can1::enableStatusChangeInterrupt(
