...
{
// ----------------------------------------------------------------------------
void UartHal8::enable() { Rcc::enable<Peripheral::Uart8>(); }

void UartHal8::disable()
{
...
void UartHal8::enableOperation() { UART8->CR1 |= USART_CR1_UE; }

void UartHal8::disableOperation() { UART8->CR1 &= ~USART_CR1_UE; }

template <class SystemClock, modm::baudrate_t baudrate, modm::percent_t tolerance>
void UartHal8::initialize(Parity parity, WordLength length)
{
    enable();
    disableOperation();
...
void UartHal8::write(uint16_t data) { UART8->DR = data; }

void UartHal8::read(uint8_t &data) { data = UART8->DR; }

void UartHal8::read(uint16_t &data) { data = UART8->DR; }

void UartHal8::setTransmitterEnable(bool enable)
{
    if (enable)
    {
...
void UartHal8::setReceiverEnable(bool enable)
{
    if (enable)
    {
...
bool UartHal8::isReceiveRegisterNotEmpty() { return UART8->SR & USART_SR_RXNE; }

bool UartHal8::isTransmitRegisterEmpty() { return UART8->SR & USART_SR_TXE; }

void UartHal8::enableInterruptVector(bool enable, uint32_t priority)
{
    if (enable)
    {
...
void UartHal8::setInterruptPriority(uint32_t priority) { NVIC_SetPriority(UART8_IRQn, priority); }

void UartHal8::enableInterrupt(Interrupt_t interrupt) { UART8->CR1 |= interrupt.value; }

void UartHal8::disableInterrupt(Interrupt_t interrupt) { UART8->CR1 &= ~interrupt.value; }

UartHal8::InterruptFlag_t UartHal8::getInterruptFlags() { return InterruptFlag_t(UART8->SR); }

void UartHal8::acknowledgeInterruptFlags(InterruptFlag_t flags)
{
