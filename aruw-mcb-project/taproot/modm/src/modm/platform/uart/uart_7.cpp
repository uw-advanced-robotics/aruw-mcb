...
{
void Uart7::writeBlocking(uint8_t data)
{
...
void Uart7::writeBlocking(const uint8_t *data, std::size_t length)
{
    while (length-- != 0)
    {
        writeBlocking(*data++);
...
void Uart7::flushWriteBuffer()
{
    while (!isWriteFinished())
        ;
}

bool Uart7::write(uint8_t data)
{
...
std::size_t Uart7::write(const uint8_t *data, std::size_t length)
{
    uint32_t i = 0;
    for (; i < length; ++i)
    {
...
bool Uart7::isWriteFinished() { return txBuffer.isEmpty() && UartHal7::isTransmitRegisterEmpty(); }

std::size_t Uart7::transmitBufferSize() { return txBuffer.getSize(); }

std::size_t Uart7::discardTransmitBuffer()
{
    std::size_t count = 0;
...
std::size_t Uart7::read(uint8_t *data, std::size_t length)
{
    uint32_t i = 0;
    for (; i < length; ++i)
    {
...
std::size_t Uart7::receiveBufferSize() { return rxBuffer.getSize(); }

std::size_t Uart7::discardReceiveBuffer()
{
    std::size_t count = 0;
