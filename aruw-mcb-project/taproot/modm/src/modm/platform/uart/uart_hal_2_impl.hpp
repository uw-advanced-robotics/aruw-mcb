...
{
// ----------------------------------------------------------------------------
void UsartHal2::enable() { Rcc::enable<Peripheral::Usart2>(); }

void UsartHal2::disable()
{
...
void UsartHal2::enableOperation() { USART2->CR1 |= USART_CR1_UE; }

void UsartHal2::disableOperation() { USART2->CR1 &= ~USART_CR1_UE; }

template <class SystemClock, modm::baudrate_t baudrate, modm::percent_t tolerance>
void UsartHal2::initialize(Parity parity, WordLength length)
{
    enable();
    disableOperation();
...
void UsartHal2::setSpiClock(SpiClock clk, LastBitClockPulse pulse)
{
...
void UsartHal2::setSpiDataMode(SpiDataMode mode)
{
    uint32_t cr2 = USART2->CR2;
...
void UsartHal2::write(uint16_t data) { USART2->DR = data; }

void UsartHal2::read(uint8_t &data) { data = USART2->DR; }

void UsartHal2::read(uint16_t &data) { data = USART2->DR; }

void UsartHal2::setTransmitterEnable(bool enable)
{
    if (enable)
    {
...
void UsartHal2::setReceiverEnable(bool enable)
{
    if (enable)
    {
...
bool UsartHal2::isReceiveRegisterNotEmpty() { return USART2->SR & USART_SR_RXNE; }

bool UsartHal2::isTransmitRegisterEmpty() { return USART2->SR & USART_SR_TXE; }

void UsartHal2::enableInterruptVector(bool enable, uint32_t priority)
{
    if (enable)
    {
...
void UsartHal2::setInterruptPriority(uint32_t priority) { NVIC_SetPriority(USART2_IRQn, priority); }

void UsartHal2::enableInterrupt(Interrupt_t interrupt) { USART2->CR1 |= interrupt.value; }

void UsartHal2::disableInterrupt(Interrupt_t interrupt) { USART2->CR1 &= ~interrupt.value; }

UsartHal2::InterruptFlag_t UsartHal2::getInterruptFlags() { return InterruptFlag_t(USART2->SR); }

void UsartHal2::acknowledgeInterruptFlags(InterruptFlag_t flags)
{
