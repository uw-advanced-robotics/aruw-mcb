...
{
// ----------------------------------------------------------------------------
void UartHal7::enable() { Rcc::enable<Peripheral::Uart7>(); }

void UartHal7::disable()
{
...
void UartHal7::enableOperation() { UART7->CR1 |= USART_CR1_UE; }

void UartHal7::disableOperation() { UART7->CR1 &= ~USART_CR1_UE; }

template <class SystemClock, modm::baudrate_t baudrate, modm::percent_t tolerance>
void UartHal7::initialize(Parity parity, WordLength length)
{
    enable();
    disableOperation();
...
void UartHal7::write(uint16_t data) { UART7->DR = data; }

void UartHal7::read(uint8_t &data) { data = UART7->DR; }

void UartHal7::read(uint16_t &data) { data = UART7->DR; }

void UartHal7::setTransmitterEnable(bool enable)
{
    if (enable)
    {
...
void UartHal7::setReceiverEnable(bool enable)
{
    if (enable)
    {
...
bool UartHal7::isReceiveRegisterNotEmpty() { return UART7->SR & USART_SR_RXNE; }

bool UartHal7::isTransmitRegisterEmpty() { return UART7->SR & USART_SR_TXE; }

void UartHal7::enableInterruptVector(bool enable, uint32_t priority)
{
    if (enable)
    {
...
void UartHal7::setInterruptPriority(uint32_t priority) { NVIC_SetPriority(UART7_IRQn, priority); }

void UartHal7::enableInterrupt(Interrupt_t interrupt) { UART7->CR1 |= interrupt.value; }

void UartHal7::disableInterrupt(Interrupt_t interrupt) { UART7->CR1 &= ~interrupt.value; }

UartHal7::InterruptFlag_t UartHal7::getInterruptFlags() { return InterruptFlag_t(UART7->SR); }

void UartHal7::acknowledgeInterruptFlags(InterruptFlag_t flags)
{
