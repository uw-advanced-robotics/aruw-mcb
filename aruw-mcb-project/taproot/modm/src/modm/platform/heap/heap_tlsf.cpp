...
extern "C"
{
    void __modm_initialize_memory(void)
    {
        ...
        {
            if (current_pool >= (mem_pools + MODM_TLSF_MAX_MEM_POOL_COUNT)) break;
    ...
    static tlsf_t get_tlsf_for_ptr(void *p)
    {
        for (mem_pool_t *pool = mem_pools; pool < (mem_pools + MODM_TLSF_MAX_MEM_POOL_COUNT);
             pool++)
        {
        ...
        modm_assert_continue_fail_debug(
            0,
            "tlsf.pool",
            "Cannot find the right TLSF pool for pointer!",
            p);
        return NULL;
    }
    ...
    try_again:
        ...
        const uint32_t clear_mask =
            (modm::MemoryTrait::TypeExternal | modm::MemoryTrait::TypeCoreCoupled).value;
        ...
        modm_assert_continue_fail_debug(0, "malloc", "No memory left in any TLSF pools!", size);
        return NULL;
    }
    ...
    void *__wrap__malloc_r(struct _reent *, size_t size)
    {
        ...
        return malloc_traits(
            size,
            uint32_t(modm::MemoryTrait::AccessSBus) | uint32_t(modm::MemoryTrait::AccessDMA));
    }
    ...
    void *__wrap__realloc_r(struct _reent *r, void *p, size_t size)
    {
        ...
        modm_assert_continue_fail_debug(ptr, "realloc", "Unable to realloc in TLSF pool!", size);
        return ptr;
    }
