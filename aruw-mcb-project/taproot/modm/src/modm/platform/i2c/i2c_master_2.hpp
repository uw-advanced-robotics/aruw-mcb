...
public:
    ...
    static void connect(PullUps pullups = PullUps::External)
    {
        ...
        static_assert(
            sizeof...(Signals) == 2 and Connector::template IsValid<Scl> and
                Connector::template IsValid<Sda>,
    ...
    static modm_always_inline void initialize()
    {
        ...
        constexpr auto result =
            Prescaler::from_range(SystemClock::I2c2 / scalar, baudrate, range_begin, 4095);
        ...
        constexpr uint32_t prescaler =
            result.prescaler | ((scalar >= 3) ? (1 << 15) : 0) | ((scalar == 25) ? (1 << 14) : 0);
        ...
        // calculate trise
        constexpr float trise_raw =
            max_rise_time < 0 ? 0 : std::floor(max_rise_time / (1'000.f / freq));
...
private:
    static void initializeWithPrescaler(
