...
#define MODM_HARDWARE_INIT_ORDER(function, order)        \
    modm_section(".hardware_init.order_" MODM_STRINGIFY( \
        order)) void* MODM_CONCAT(__modm_hardware_init_ptr_, function) = (void*)&function
...
#define MODM_HARDWARE_INIT_NAME_ORDER(name, function, order) \
    modm_section(".hardware_init.order_" MODM_STRINGIFY(     \
        order)) void* MODM_CONCAT(__modm_hardware_init_ptr_, name) = (void*)&function
