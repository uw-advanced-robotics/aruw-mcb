...
xpcc::CanConnector<Driver>::CanConnector(Driver *driver) : canDriver(driver)
{
}
...
// ----------------------------------------------------------------------------
template <typename Driver>
bool xpcc::CanConnector<Driver>::isPacketAvailable() const
{
    return !this->receivedMessages.isEmpty();
}

template <typename Driver>
const xpcc::Header &xpcc::CanConnector<Driver>::getPacketHeader() const
{
    return this->receivedMessages.getFront().header;
}

template <typename Driver>
const modm::SmartPointer xpcc::CanConnector<Driver>::getPacketPayload() const
{
...
// ----------------------------------------------------------------------------
template <typename Driver>
void xpcc::CanConnector<Driver>::sendPacket(const Header &header, modm::SmartPointer payload)
{
    ...
    if (!fragmented && this->canDriver->isReadyToSend())
    {
        ...
        successful = this->sendMessage(identifier, payload.getPointer(), payload.getSize());
    }
...
// ----------------------------------------------------------------------------
template <typename Driver>
void xpcc::CanConnector<Driver>::dropPacket()
{
...
// ----------------------------------------------------------------------------
template <typename Driver>
void xpcc::CanConnector<Driver>::update()
{
...
template <typename Driver>
bool xpcc::CanConnector<Driver>::sendMessage(
...
template <typename Driver>
void xpcc::CanConnector<Driver>::sendWaitingMessages()
{
    ...
    {
        if (this->sendMessage(message.identifier, message.payload.getPointer(), messageSize))
        {
            this->sendList.removeFront();
        }
    }
...
template <typename Driver>
bool xpcc::CanConnector<Driver>::retrieveMessage()
{
    ...
    if (this->canDriver->getMessage(message))
    {
        ...
        if (!isFragment)
        {
            ...
            std::memcpy(
                this->receivedMessages.getBack().payload.getPointer(),
                message.data,
                message.length);
        }
        else
        {
            ...
            if (message.length < 3 || messageSize > 48 || fragmentIndex >= numberOfFragments)
            {
                // illegal format:
            ...
            {
                if (packet->header == header && packet->counter == counter)
                {
                    break;
                }
            }
            ...
            std::memcpy(
                packet->payload.getPointer() + offset,
                message.data + 2,
                message.length - 2);
