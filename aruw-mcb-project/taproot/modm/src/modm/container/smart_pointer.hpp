...
public:
    ...
    template <typename T>
    explicit SmartPointer(const T* data) : ptr(new uint8_t[sizeof(T) + 4])
    {
        ptr[0] = 1;
    ...
    inline const uint8_t* getPointer() const { return ptr + 4; }

    inline uint8_t* getPointer() { return ptr + 4; }

    inline uint16_t getSize() const { return *reinterpret_cast<uint16_t*>(ptr + 2); }
...
protected:
    friend IOStream& operator<<(IOStream&, const SmartPointer&);
} modm_packed;

modm::IOStream& operator<<(modm::IOStream& s, const modm::SmartPointer& sPtr);
