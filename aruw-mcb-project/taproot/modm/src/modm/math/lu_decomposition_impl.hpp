...
bool modm::LUDecomposition::decompose(
...
bool modm::LUDecomposition::solve(
...
// ----------------------------------------------------------------------------
template <typename T, uint8_t SIZE>
void modm::LUDecomposition::RowOperation<T, SIZE>::multiply(
    T *intoRow,
    const T *row,
    const T &factor)
{
...
// ----------------------------------------------------------------------------
template <typename T, uint8_t SIZE>
void modm::LUDecomposition::RowOperation<T, SIZE>::addRowTimesFactor(
    T *intoRow,
    const T *srcRow,
    const T *addRow,
    const T &timesFactor)
...
// ----------------------------------------------------------------------------
template <typename T, uint8_t SIZE>
void modm::LUDecomposition::RowOperation<T, SIZE>::swap(T *row1, T *row2)
{
    T tmp[SIZE];
...
// ----------------------------------------------------------------------------
template <typename T>
void modm::LUDecomposition::RowOperation<T, 0>::multiply(
    T * /* intoRow */,
    const T * /* row */,
    const T & /* factor */)
{
...
// ----------------------------------------------------------------------------
template <typename T>
void modm::LUDecomposition::RowOperation<T, 0>::addRowTimesFactor(
    T * /* intoRow */,
    const T * /* srcRow */,
    const T * /* addRow */,
    const T & /* timesFactor */)
...
// ----------------------------------------------------------------------------
template <typename T>
void modm::LUDecomposition::RowOperation<T, 0>::swap(T * /* row1 */, T * /* row2 */)
{
}
...
bool modm::LUDecomposition::LUSubDecomposition<T, OFFSET, HEIGHT, WIDTH>::decomposeRecur(T *u, T *l)
{
    if (!decompose(u, l)) return false;
...
// ----------------------------------------------------------------------------
template <typename T, uint8_t OFFSET, uint8_t HEIGHT, uint8_t WIDTH>
bool modm::LUDecomposition::LUSubDecomposition<T, OFFSET, HEIGHT, WIDTH>::decompose(T *u, T *l)
{
    ...
    RowOperation<T, WIDTH - OFFSET - 1>::multiply(
        &u[OFFSET * width + OFFSET + 1],
        &u[OFFSET * width + OFFSET + 1],
        T(1.0) / factor);
    ...
    for (uint_fast8_t j = OFFSET + 1; j < height; ++j)
    {
        ...
        u[j * width + OFFSET] = 0;
        RowOperation<T, WIDTH - OFFSET - 1>::addRowTimesFactor(
            &u[j * width + OFFSET + 1],
            &u[j * width + OFFSET + 1],
            &u[OFFSET * width + OFFSET + 1],
            -factor);
...
// ----------------------------------------------------------------------------
template <typename T, uint8_t OFFSET, uint8_t HEIGHT, uint8_t WIDTH>
bool modm::LUDecomposition::LUSubDecomposition<T, OFFSET, HEIGHT, WIDTH>::decomposeRecur(
    T *u,
    T *l,
    int8_t *p)
{
    if (!decompose(u, l, p)) return false;
...
// ----------------------------------------------------------------------------
template <typename T, uint8_t OFFSET, uint8_t HEIGHT, uint8_t WIDTH>
bool modm::LUDecomposition::LUSubDecomposition<T, OFFSET, HEIGHT, WIDTH>::decompose(
    T *u,
    T *l,
    int8_t *p)
{
    ...
    if (maxRow != OFFSET)
    {
        ...
        p[OFFSET] = p[maxRow];
        p[maxRow] = temp;
        RowOperation<T, WIDTH - OFFSET>::swap(
            &u[maxRow * width + OFFSET],
            &u[OFFSET * width + OFFSET]);
...
bool modm::LUDecomposition::LUSubDecomposition<T, OFFSET, HEIGHT, WIDTH>::solveLyEqualsB(
...
template <typename T, uint8_t OFFSET, uint8_t HEIGHT, uint8_t WIDTH>
template <uint8_t BXWIDTH>
bool modm::LUDecomposition::LUSubDecomposition<T, OFFSET, HEIGHT, WIDTH>::solveUxEqualsY(
...
template <typename T, uint8_t OFFSET, uint8_t HEIGHT, uint8_t WIDTH>
template <uint8_t BXWIDTH>
bool modm::LUDecomposition::LUSubDecomposition<T, OFFSET, HEIGHT, WIDTH>::solve(
...
bool modm::LUDecomposition::LUSubDecomposition<T, OFFSET, WIDTH, OFFSET>::decomposeRecur(
    T * /* u */,
    T * /* l */,
    int8_t * /* p */)
{
...
template <typename T, uint8_t OFFSET, uint8_t WIDTH>
template <uint8_t BXWIDTH>
bool modm::LUDecomposition::LUSubDecomposition<T, OFFSET, WIDTH, OFFSET>::solveUxEqualsY(
...
template <typename T, uint8_t OFFSET, uint8_t WIDTH>
template <uint8_t BXWIDTH>
bool modm::LUDecomposition::LUSubDecomposition<T, OFFSET, WIDTH, OFFSET>::solveLyEqualsB(
