...
// -----------------------------------------------------------------------------
template <typename T, int N, int BLOCK_SIZE, signed int ScaleFactor>
void modm::filter::Fir<T, N, BLOCK_SIZE, ScaleFactor>::reset()
{
...
// -----------------------------------------------------------------------------
template <typename T, int N, int BLOCK_SIZE, signed int ScaleFactor>
void modm::filter::Fir<T, N, BLOCK_SIZE, ScaleFactor>::append(const T& input)
{
#ifdef FIR_DEBUG_APPEND
    printf("append(%f);\ttaps_index=%d\n", input, taps_index);
    printf("BEFORE: ");
    for (int i = 0; i < N + BLOCK_SIZE; i++)
        if (i >= taps_index && i - taps_index < N)
            printf("n-%d:%.3f; ", i - taps_index, taps[i]);
        else
            printf("(%.3f)    ", taps[i]);
    printf("\n");
    ...
    {
        for (int i = N + BLOCK_SIZE - 1; i > BLOCK_SIZE; i--) taps[i] = taps[i - BLOCK_SIZE - 1];
        taps_index = BLOCK_SIZE;
    }
