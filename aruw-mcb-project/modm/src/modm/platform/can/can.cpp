		...
		uint32_t interruptPriority, Mode startupMode, bool overwriteOnOverrun)
{
	...
	if (overwriteOnOverrun) {
		...
		CAN->MCR |= CAN_MCR_ABOM | CAN_MCR_RFLM | CAN_MCR_TXFP;
	}

	// Request initialization
	CAN->MCR |= CAN_MCR_INRQ;
	int deadlockPreventer = 10'000; // max ~10ms
	while (((CAN->MSR & CAN_MSR_INAK) == 0) and (deadlockPreventer-- > 0)) {
	...
	modm_assert(deadlockPreventer > 0, "can", "init", "timeout", 0);

	// Enable Interrupts:
	// FIFO1 Overrun, FIFO0 Overrun
	CAN->IER = CAN_IER_FOVIE1 | CAN_IER_FOVIE0;
	...
	CAN->IER |= CAN_IER_FMPIE1 | CAN_IER_FMPIE0;
	CAN->BTR =
	...
	while (((CAN->MSR & CAN_MSR_INAK) == CAN_MSR_INAK) and (deadlockPreventer-- > 0))  {
		// wait for the normal mode
	}
	modm_assert(deadlockPreventer > 0, "can", "init", "timeout2", 0);
}
...
{
	CAN_TxMailBox_TypeDef* mailbox = &CAN->sTxMailBox[mailboxId];
...
{
	CAN_FIFOMailBox_TypeDef* mailbox = &CAN->sFIFOMailBox[mailboxId];
...
MODM_ISR(CAN_RX0)
{
	if (CAN->RF0R & CAN_RF0R_FOVR0) {
		modm::ErrorReport::report(modm::platform::CAN_FIFO0_OVERFLOW);

		// release overrun flag & access the next message
		CAN->RF0R = CAN_RF0R_FOVR0 | CAN_RF0R_RFOM0;
	}
...
MODM_ISR(CAN_RX1)
{
	if (CAN->RF1R & CAN_RF1R_FOVR1) {
		modm::ErrorReport::report(modm::platform::CAN_FIFO1_OVERFLOW);

		// release overrun flag & access the next message
		CAN->RF1R = CAN_RF1R_FOVR1 | CAN_RF1R_RFOM1;
	}
...
MODM_ISR(CEC_CAN)
{
	if(CAN->TSR & (CAN_TSR_RQCP0 | CAN_TSR_RQCP1 | CAN_TSR_RQCP2)) {
		MODM_ISR_CALL(CAN_TX);
	}

	if(CAN->RF0R & (CAN_RF0R_FMP0 | CAN_RF0R_FULL0 | CAN_RF0R_FOVR0)) {
		MODM_ISR_CALL(CAN_RX0);
	}

	if(CAN->RF1R & (CAN_RF1R_FMP1 | CAN_RF1R_FULL1 | CAN_RF1R_FOVR1)) {
		MODM_ISR_CALL(CAN_RX1);
	}
...
modm::platform::Can::setMode(Mode mode)
{
	...
	CAN->BTR = (CAN->BTR & ~(CAN_BTR_SILM | CAN_BTR_LBKM))
...
// ----------------------------------------------------------------------------
bool
modm::platform::Can::getMessage(can::Message& message, uint8_t *filter_id)
{
	if (rxQueue.isEmpty())
	{
...
modm::platform::Can::sendMessage(const can::Message& message)
{
	...
	if ((CAN->TSR & (CAN_TSR_TME0 | CAN_TSR_TME1 | CAN_TSR_TME2)) == 0)
	{
		...
		uint32_t mailbox = (CAN->TSR & CAN_TSR_CODE) >> 24;
...
// ----------------------------------------------------------------------------
void
modm::platform::Can::enableStatusChangeInterrupt(
		uint32_t interruptEnable,
		uint32_t
)
{
	CAN->IER = interruptEnable | (CAN->IER & 0x000000ff);
}