...
uint16_t modm_fastdata ns_per_loop(250);
}
			...
			uint8_t pllMul, uint8_t pllPrediv, uint32_t waitCycles)
{
	...
	uint32_t tmp = RCC->CFGR & ~(RCC_CFGR_USBPRE | RCC_CFGR_PLLMUL | RCC_CFGR_PLLSRC | RCC_CFGR_PLLXTPRE);
	...
	tmp |= (static_cast<uint32_t>(pllMul - 2) << 18) & RCC_CFGR_PLLMUL;

#ifdef RCC_CFGR2_PREDIV
	// HSE PREDIV division factor
	RCC->CFGR2 = (RCC->CFGR2 & ~(RCC_CFGR2_PREDIV)) | (uint32_t(pllPrediv - 1) & RCC_CFGR2_PREDIV);
#else
