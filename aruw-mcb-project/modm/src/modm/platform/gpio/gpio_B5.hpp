...
public:
	...
	struct Mosi { static void connect();
		static_assert(
			...
			"GpioB5::Mosi only connects to Spi1!");
	};
	template< Peripheral peripheral >
	struct Sd { static void connect();
		static_assert(
			(peripheral == Peripheral::I2s1),
			"GpioB5::Sd only connects to I2s1!");
	};
	template< Peripheral peripheral >
	...
	template< Peripheral peripheral >
	struct Wkup6 { static void connect();
		static_assert(
			(peripheral == Peripheral::Sys),
			"GpioB5::Wkup6 only connects to Sys!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
template<>
struct GpioB5::Wkup6<Peripheral::Sys>
{
	using Gpio = GpioB5;
