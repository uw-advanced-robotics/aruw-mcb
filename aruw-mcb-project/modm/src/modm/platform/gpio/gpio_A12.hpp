...
public:
	...
	struct G4Io4 { static void connect();
		static_assert(
			...
			"GpioA12::G4Io4 only connects to Tsc!");
	};
	template< Peripheral peripheral >
	struct Out { static void connect();
		static_assert(
			(peripheral == Peripheral::Comp2),
			"GpioA12::Out only connects to Comp2!");
	};
	template< Peripheral peripheral >
	...
	struct Tx { static void connect();
		static_assert(
			(peripheral == Peripheral::Can),
			"GpioA12::Tx only connects to Can!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
template<>
struct GpioA12::G4Io4<Peripheral::Tsc>
{
	using Gpio = GpioA12;
...
template<>
struct GpioA12::Out<Peripheral::Comp2>
{
	using Gpio = GpioA12;
...
template<>
struct GpioA12::Tx<Peripheral::Can>
{
	using Gpio = GpioA12;
