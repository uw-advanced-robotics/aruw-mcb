...
public:
	...
	struct Ch1 { static void connect();
		static_assert(
			...
			"GpioA4::Ch1 only connects to Tim14!");
	};
	template< Peripheral peripheral >
	...
	struct G2Io1 { static void connect();
		static_assert(
			...
			"GpioA4::G2Io1 only connects to Tsc!");
	};
	template< Peripheral peripheral >
	struct In4 { static void connect();
		static_assert(
			(peripheral == Peripheral::Adc),
			"GpioA4::In4 only connects to Adc!");
	};
	template< Peripheral peripheral >
	...
	struct Nss { static void connect();
		static_assert(
			...
			"GpioA4::Nss only connects to Spi1!");
	};
	template< Peripheral peripheral >
	...
	struct Ws { static void connect();
		static_assert(
			...
			"GpioA4::Ws only connects to I2s1!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
template<>
struct GpioA4::Ch1<Peripheral::Tim14>
{
	using Gpio = GpioA4;
...
template<>
struct GpioA4::G2Io1<Peripheral::Tsc>
{
	using Gpio = GpioA4;
...
template<>
struct GpioA4::In4<Peripheral::Adc>
{
	using Gpio = GpioA4;
...
template<>
constexpr int8_t
GpioA4::AdcChannel<Peripheral::Adc> = 4;
template<>
