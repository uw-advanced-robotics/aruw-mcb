...
public:
	...
	struct In12 { static void connect();
		static_assert(
			...
			"GpioC2::In12 only connects to Adc!");
	};
	template< Peripheral peripheral >
	struct Mck { static void connect();
		static_assert(
			(peripheral == Peripheral::I2s2),
			"GpioC2::Mck only connects to I2s2!");
	};
	template< Peripheral peripheral >
...
template<>
struct GpioC2::In12<Peripheral::Adc>
{
	using Gpio = GpioC2;
...
template<>
constexpr int8_t
GpioC2::AdcChannel<Peripheral::Adc> = 12;
template<>
struct GpioC2::Mck<Peripheral::I2s2>
{
	using Gpio = GpioC2;
