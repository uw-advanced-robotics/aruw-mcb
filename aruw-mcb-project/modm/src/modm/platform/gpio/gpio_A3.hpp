...
public:
	...
	struct G1Io4 { static void connect();
		static_assert(
			...
			"GpioA3::G1Io4 only connects to Tsc!");
	};
	template< Peripheral peripheral >
	struct In3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Adc),
			"GpioA3::In3 only connects to Adc!");
	};
	template< Peripheral peripheral >
...
template<>
struct GpioA3::G1Io4<Peripheral::Tsc>
{
	using Gpio = GpioA3;
...
template<>
struct GpioA3::In3<Peripheral::Adc>
{
	using Gpio = GpioA3;
...
template<>
constexpr int8_t
GpioA3::AdcChannel<Peripheral::Adc> = 3;
template<>
