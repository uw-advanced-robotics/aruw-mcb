...
public:
	...
	struct G3Io2 { static void connect();
		static_assert(
			...
			"GpioB0::G3Io2 only connects to Tsc!");
	};
	template< Peripheral peripheral >
	struct In8 { static void connect();
		static_assert(
			(peripheral == Peripheral::Adc),
			"GpioB0::In8 only connects to Adc!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
template<>
struct GpioB0::G3Io2<Peripheral::Tsc>
{
	using Gpio = GpioB0;
...
template<>
struct GpioB0::In8<Peripheral::Adc>
{
	using Gpio = GpioB0;
...
template<>
constexpr int8_t
GpioB0::AdcChannel<Peripheral::Adc> = 8;
/// @endcond
