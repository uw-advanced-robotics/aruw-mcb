...
public:
	...
	struct Ch1 { static void connect();
		static_assert(
			...
			"GpioA7::Ch1 only connects to Tim14 or Tim17!");
	};
	template< Peripheral peripheral >
	...
	struct G2Io4 { static void connect();
		static_assert(
			...
			"GpioA7::G2Io4 only connects to Tsc!");
	};
	template< Peripheral peripheral >
	struct In7 { static void connect();
		static_assert(
			(peripheral == Peripheral::Adc),
			"GpioA7::In7 only connects to Adc!");
	};
	template< Peripheral peripheral >
...
template<>
struct GpioA7::G2Io4<Peripheral::Tsc>
{
	using Gpio = GpioA7;
...
template<>
struct GpioA7::In7<Peripheral::Adc>
{
	using Gpio = GpioA7;
...
template<>
constexpr int8_t
GpioA7::AdcChannel<Peripheral::Adc> = 7;
template<>
