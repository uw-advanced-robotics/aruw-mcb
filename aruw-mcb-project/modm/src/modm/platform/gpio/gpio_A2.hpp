...
public:
	...
	struct G1Io3 { static void connect();
		static_assert(
			...
			"GpioA2::G1Io3 only connects to Tsc!");
	};
	template< Peripheral peripheral >
	struct In2 { static void connect();
		static_assert(
			(peripheral == Peripheral::Adc),
			"GpioA2::In2 only connects to Adc!");
	};
	template< Peripheral peripheral >
...
template<>
struct GpioA2::G1Io3<Peripheral::Tsc>
{
	using Gpio = GpioA2;
...
template<>
struct GpioA2::In2<Peripheral::Adc>
{
	using Gpio = GpioA2;
...
template<>
constexpr int8_t
GpioA2::AdcChannel<Peripheral::Adc> = 2;
template<>
