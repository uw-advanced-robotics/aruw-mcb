...
#include "spi_master_4.hpp"

// Bit0: single transfer state
// Bit1: block transfer state
uint8_t
modm::platform::SpiMaster4::state(0);

uint8_t
modm::platform::SpiMaster4::count(0);

void *
modm::platform::SpiMaster4::context(nullptr);

modm::Spi::ConfigurationHandler
modm::platform::SpiMaster4::configuration(nullptr);
// ----------------------------------------------------------------------------

uint8_t
modm::platform::SpiMaster4::acquire(void *ctx, ConfigurationHandler handler)
{
	if (context == nullptr)
	{
		context = ctx;
...
uint8_t
modm::platform::SpiMaster4::release(void *ctx)
{
	if (ctx == context)
	{
		if (--count == 0)
...
modm::platform::SpiMaster4::transfer(uint8_t data)
{
	...
	if ( !(state & Bit0) )
	{
		...
		if (!SpiHal4::isTransmitRegisterEmpty())
	...
	if (!SpiHal4::isReceiveRegisterNotEmpty())
		return {modm::rf::Running};
...
modm::platform::SpiMaster4::transfer(
