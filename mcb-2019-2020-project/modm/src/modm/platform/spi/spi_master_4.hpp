...
#ifndef MODM_STM32_SPI_MASTER4_HPP
#define MODM_STM32_SPI_MASTER4_HPP

#include <modm/architecture/interface/spi_master.hpp>
#include <modm/platform/gpio/connector.hpp>
#include "spi_hal_4.hpp"
 ...
 * @ingroup	modm_platform_spi modm_platform_spi_4
 */
class SpiMaster4 : public modm::SpiMaster
{
...
public:
	...
	connect()
	{
		using Connector = GpioConnector<Peripheral::Spi4, Signals...>;
	...
	initialize()
	{
		...
		constexpr float desired = float(SystemClock::Spi4) / baudrate;
		...
		assertBaudrateInTolerance<
				SystemClock::Spi4 / nearest,
				baudrate,
				tolerance >();

		// translate the prescaler into the bitmapping
		constexpr SpiHal4::Prescaler prescaler = (
				(nearest >= 256) ? SpiHal4::Prescaler::Div256 : (
				(nearest >= 128) ? SpiHal4::Prescaler::Div128 : (
				(nearest >=  64) ? SpiHal4::Prescaler::Div64  : (
				(nearest >=  32) ? SpiHal4::Prescaler::Div32  : (
				(nearest >=  16) ? SpiHal4::Prescaler::Div16  : (
				(nearest >=   8) ? SpiHal4::Prescaler::Div8   : (
				(nearest >=   4) ? SpiHal4::Prescaler::Div4   :
								   SpiHal4::Prescaler::Div2
				)))))));

		// initialize the Spi
		SpiHal4::initialize(prescaler);
		state = 0;
	}
	...
	{
		SpiHal4::setDataMode(static_cast<SpiHal4::DataMode>(mode));
	}

	static modm_always_inline void
	setDataOrder(DataOrder order)
	{
		SpiHal4::setDataOrder(static_cast<SpiHal4::DataOrder>(order));
	}
...
#endif // MODM_STM32_SPI_MASTER4_HPP