...
namespace sensors
{
    bool Ist8310::caliIst = false;

    Ist8310::Ist Ist8310::ist = {0, 0, 0, 0, 0, 0};
    ...
    void Ist8310::istRegWriteByMpu(uint8_t addr, uint8_t data)
    {
        ...
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV1_CTRL, 0x00);
        modm::delayMilliseconds(2);
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV1_REG, addr);
        modm::delayMilliseconds(2);
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV1_DO, data);
        modm::delayMilliseconds(2);
        //turn on slave 1 with one byte transmitting
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV1_CTRL, 0x80 | 0x01);
    ...
    {
        uint8_t retval;
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV4_REG, addr);
        modm::delayMilliseconds(10);
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV4_CTRL, 0x80);
        modm::delayMilliseconds(10);
        retval = Mpu6500::mpuReadReg(MPU6500_I2C_SLV4_DI);
        //turn off slave4 after read
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV4_CTRL, 0x00);
        modm::delayMilliseconds(10);
        return retval;
    }
    ...
    void Ist8310::mpuMstI2cAutoReadConfig(uint8_t device_address, uint8_t reg_base_addr, uint8_t data_num)
    {
        ...
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV1_ADDR, device_address);
        modm::delayMilliseconds(2);
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV1_REG, IST8310_R_CONFA);
        modm::delayMilliseconds(2);
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV1_DO, IST8310_ODR_MODE);
        modm::delayMilliseconds(2);

        //use slave0,auto read data
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV0_ADDR, 0x80 | device_address);
        modm::delayMilliseconds(2);
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV0_REG, reg_base_addr);
        modm::delayMilliseconds(2);

        //every eight mpu6500 internal samples one i2c master read
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV4_CTRL, 0x03);
        modm::delayMilliseconds(2);
        //enable slave 0 and 1 access delay
        Mpu6500::mpuWriteReg(MPU6500_I2C_MST_DELAY_CTRL, 0x01 | 0x02);
        modm::delayMilliseconds(2);
        //enable slave 1 auto transmit
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV1_CTRL, 0x80 | 0x01);
        modm::delayMilliseconds(6); //Wait 6ms (minimum waiting time for 16 times internal average setup)
        //enable slave 0 with data_num bytes reading
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV0_CTRL, 0x80 | data_num);
        modm::delayMilliseconds(2);
    }
    ...
    uint8_t Ist8310::ist8310Init(void)
    {
        ...
        Mpu6500::mpuWriteReg(MPU6500_USER_CTRL, 0x30);
        modm::delayMilliseconds(10);
        //I2C master clock 400kHz
        Mpu6500::mpuWriteReg(MPU6500_I2C_MST_CTRL, 0x0d);
        modm::delayMilliseconds(10);

        //turn on slave 1 for ist write and slave 4 for ist read
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV1_ADDR, IST8310_ADDRESS); //write ist
        modm::delayMilliseconds(10);
        Mpu6500::mpuWriteReg(MPU6500_I2C_SLV4_ADDR, 0x80 | IST8310_ADDRESS); //read ist
        modm::delayMilliseconds(10);
    ...
    {
        uint8_t magData[6];
        Mpu6500::mpuReadRegs(MPU6500_EXT_SENS_DATA_00, magData, 6);
