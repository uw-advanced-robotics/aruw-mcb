...
class XavierSerial : DJISerial
{
 private:
    ...
    static const uint8_t CV_MESSAGE_TYPE_ROBOT_ID         = 0x03;
    static const uint8_t CV_MESSAGE_TYPE_REQUEST_TASK     = 0x04;
    static const uint8_t CV_MESSAGE_TYPE_AUTO_AIM_REQUEST = 0x05;

    static const uint8_t CV_MESSAGE_TYPE_SIZE = 5;

    // RX message headers
    static const int16_t TIME_OFFLINE_CV_AIM_DATA_MS = 5000;  // time in ms since last CV aim data was
                                                              // received before deciding CV is offline

                                                              // todo(matthew) add this in
 ...
 public:
    ...
    typedef void (*TurretDataHandler)(TurretAimData* aimData);
     ...
     * and the callback
     */
    void initializeCV(TurretDataHandler turretDataCallback);
 ...
 private:
    ...
    TurretDataHandler turretDataHandler;
