...
void Mpu6500::init()
{
    Drivers::spi.initialize<Board::SystemClock, 703125_Hz>(communication::Spi::SpiPort::SPI_5);

    // set power mode
    Drivers::spi.writeRegister(communication::Spi::SpiPort::SPI_5, MPU6500_PWR_MGMT_1, 0x80);

    modm::delayMilliseconds(100);

    // reset gyro, accel, and temp
    Drivers::spi.writeRegister(communication::Spi::SpiPort::SPI_5, MPU6500_SIGNAL_PATH_RESET, 0x07);
    modm::delayMilliseconds(100);

    // verify mpu register ID
    if (MPU6500_ID !=
        Drivers::spi.readRegister(communication::Spi::SpiPort::SPI_5, MPU6500_WHO_AM_I))
    {
        RAISE_ERROR(
    ...
    for (int i = 0; i < 7; i++)
    {
        Drivers::spi.writeRegister(
            communication::Spi::SpiPort::SPI_5,
            Mpu6500InitData[i][0],
            Mpu6500InitData[i][1]);
...
void Mpu6500::read()
{
    if (imuInitialized)
    {
        Drivers::spi.readRegisters(
            communication::Spi::SpiPort::SPI_5,
            MPU6500_ACCEL_XOUT_H,
            rxBuff,
            ACC_GYRO_TEMPERATURE_BUFF_RX_SIZE);
...
void Mpu6500::calculateGyroOffset()
{
    ...
    {
        Drivers::spi
            .readRegisters(communication::Spi::SpiPort::SPI_5, MPU6500_ACCEL_XOUT_H, rxBuff, 14);
